...
function Postsfromfollowing() {
    ...
    const [name,setlikename] = useState("");
    ...
    useEffect(() => {
        if(posts){
            fetch("https://instabackend22.herokuapp.com/Postsfromfollowing",{
                method: "get",
                headers:
                {
    ...
    return ( 
        ...
        state.following.length > 0 ?  posts.length > 0 && posts.length!=0 ? 
        posts.map((post)=>{
            return(   
  ...
  : <i onClick={()=> likeunlike(post._id,'https://instabackend22.herokuapp.com/like')}className="material-icons" style={{color: "black",marginRight: "1px",cursor:"pointer"}} ><FavoriteBorder /></i>
                          ...
                          <h6 style={{ color: "darkblue", cursor: "pointer", fontSize: '10px', float: 'right', margin: "-10px" }} className=''> {timeconverter(post.createdAt)} </h6>
                      </div>
